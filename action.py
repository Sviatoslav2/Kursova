...
print([i for i in data_dct])
###########################################################################

###########################################################################
def way_through(begin,data_dct):
    '''
    ...
    for i in range(len([i for i in a.dct])):
        if point not in lst1:
            ...
            sum_len +=a.min_len_to_point(point)
        else :
...
def main():
    ...
    graph_len = dijkstra_shortest_path_len(graph, start, p={}, u=[], d={})
    key = input("Enter the name of regime(way_through,get_way_to_finish,get_way_to_finish_by_rad) : ",)
    if key == "way_through":
        return way_through(start,data_dct)
