...
def way_through_all(begin,data_dct):
    ...
    for i in range(len(a)):
        point = a.next_point(lst[-1])
        if a.is_in_grahp(point):
            if point not in lst:
                lst.append(point)
                sum_len += a.min_len_to_point(lst[-1])
        else:
            if point not in lst:
                lst.append(a.min_len_to_point(lst[-1],1)[1])
                sum_len += a.min_len_to_point(lst[-1],1)[0]
    lst.remove(begin)
    return (lst,sum_len)
